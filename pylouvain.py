...
class PyLouvain:
    ...
    def __init__(self, nodes, edges):
        ...
        self.m = 0 ### la somme des poids des arretes du graphe (la somme total des arretes dans un graphe non pondere)
        self.k_i = [0 for n in nodes] ## somme totale des arretes incidentes au noeud i (degre de chaque noeud)
        self.edges_of_node = {}
        ...
        for e in edges:
            ...
            # save edges by node
            if e[0][0] not in self.edges_of_node: ### dictionnaire pour chaque noeuds la liste de ses arretes 
    ...
    def apply_method(self):
        ...
        best_partition = [[node] for node in network[0]] #### chaque noeud dans une communaute
        best_q = -1
        i = 1
        while 1:
            #print("pass #%d" % i)
            ...
            if q == best_q: ### arret quand q ne bouge plus
                break
    ...
    '''
        ...
        stot: degre total des noeuds dans c
    '''
    ...
    def first_phase(self, network):
        ...
        while 1:
            ...
            for node in network[0]:
                ...
                for neighbor in self.get_neighbors(node): #### parcour toutes les communautes des noeuds voisins et voir celui qui maximise la modularite gain
    ...
    '''
        ...
        _node: an int
    '''
    def get_neighbors(self, node): ## renvoi tous les noeuds voisins au noeud node
        for e in self.edges_of_node[node]:
    ...
    def make_initial_partition(self, network):
        ...
        self.s_in = [0 for node in network[0]] ## degre de chaque noeud a linterieur de la communaute c pr sa division par 2m
        self.s_tot = [self.k_i[node] for node in network[0]] ### degre de chaque noeud
        for e in network[1]:
    ...
    def second_phase(self, network, partition):
        nodes_ = [i for i in range(len(partition))] #### les noeuds cest les communautes
